...
DECLARE  
	...
	v_task_detail_name TEXT := 'Play Daily Trivia';
    ...
    v_year INT := 2025;
    v_current_date DATE := make_date(v_year, 1, 1);
    v_next_date DATE;
    v_end_date DATE := make_date(v_year, 12, 30);  -- Dec 30 to avoid 12-31 + 1
    ...
    BEGIN
        ...
        IF v_task_reward_code IS NULL THEN
			...
			WHILE v_current_date <= v_end_date LOOP
				v_next_date := v_current_date + 1;

				v_schedule := v_schedule || jsonb_build_object(
					'startDate', to_char(v_current_date, 'MM-DD'),
					'expiryDate', to_char(v_next_date, 'MM-DD')
				);
