...
namespace SunnyRewards.Helios.Admin.Api.Controllers
{
    ...
    public class TaskDetailsController : ControllerBase
    {
        ...
        private const string className = nameof(TaskDetailsController);
        ...
        public async Task<IActionResult> CreateTaskDetails(CreateTaskDetailsRequestDto requestDto)
        {
            ...
            try
            {
                ...
                {
                    _taskDetailsLogger.LogError("{ClassName}.{MethodName}: Error occurred while creating Task Detail. Request: {RequestData}, Response: {ResponseData}, ErrorCode: {ErrorCode}", className, methodName, requestDto.ToJson(), response.ToJson(), response.ErrorCode);
                    return StatusCode((int)response.ErrorCode, response);
                }

                _taskDetailsLogger.LogInformation("{ClassName}.{MethodName}: Task details created Successful, with TenantCode: {TenantCode}", className, methodName, requestDto.TaskDetail.TenantCode);

                return Ok(response);
            }
            catch (Exception ex)
            {
                _taskDetailsLogger.LogError(ex, "{ClassName}.{MethodName}: An error occurred while create Task Detail. Error Message: {ErrorMessage}, ErrorCode: {ErrorCode}", className, methodName, ex.Message, StatusCodes.Status500InternalServerError);
