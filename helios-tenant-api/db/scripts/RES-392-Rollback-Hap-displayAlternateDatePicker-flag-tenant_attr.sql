-- ============================================================================
-- 🚀 Script    : Script to Update flag "displayAlternateDatePicker" in tenant_attr column with default value False
-- 📌 Purpose   : Rollback the flag to default value False
-- 🧑 Author    : Siva Krishna
-- 📅 Date      : 29-09-2025
-- 🧾 Jira      : RES-392 & RES-561(Sub-task)
-- ⚠️ Inputs    : HAP-TENANT-CODE
-- 📤 Output    : It will Rollback(update to default value) the flag "displayAlternateDatePicker" to 'false' for input tenant in tenant.tenant
-- 🔗 Script URL: NA
-- 📝 Notes     : NA
-- ============================================================================
...
DECLARE
    ...
    v_exists BOOLEAN;
BEGIN
    RAISE NOTICE '[Information] Starting rollback: Attempting to set "displayAlternateDatePicker"=false for tenant_code=%', v_tenant_code;
    ...
    SET tenant_attr = tenant_attr || jsonb_build_object('displayAlternateDatePicker', false)
    ...
    IF v_updated_count > 0 THEN
        RAISE NOTICE '[Information] Successfully rolled back: "displayAlternateDatePicker"=false for tenant_code=%', v_tenant_code;
    ELSE
