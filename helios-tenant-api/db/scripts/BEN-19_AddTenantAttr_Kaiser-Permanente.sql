...
DECLARE
  -- === INPUTS ===
  v_tenant_code TEXT := '<KP-TENANT-CODE>';  -- <-- CHANGE ME
  v_env         TEXT := 'env';                                    -- <-- one of DEV/QA/UAT/INTEG/PROD
...
BEGIN
  ...
  v_icon_url  := v_img_base || '/kp_mobile_icon.png';
  v_desktop_icon_url  := v_img_base || '/kp_desktop_icon.png';
  v_check_url := v_img_base || '/check-circle-background.png';
  ...
  SET tenant_attr = jsonb_set(
      ...
      '{headerImageUrls}',
      jsonb_build_object(
        'headerMobileIconUrl',  v_icon_url,
        'headerDesktopIconUrl', v_desktop_icon_url
      ),
      true
  )
  WHERE tenant_code = v_tenant_code AND delete_nbr = 0;
  ...
  SET tenant_attr = jsonb_set(
      ...
      '{includeLanguageDropdown}', 'true'::jsonb, true
  )
  WHERE tenant_code = v_tenant_code AND delete_nbr = 0;
