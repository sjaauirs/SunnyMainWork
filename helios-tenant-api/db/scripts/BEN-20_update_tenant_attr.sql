-- ===================================================================================
-- Purpose       : Add/Update radioActiveBgColor,onboardingSurvey and radioInActiveBgColor from tenant_attr JSONB
...
BEGIN
    ...
    IF (v_new_attr #>> '{ux,commonColors,radioActiveBgColor}') IS NULL THEN
        v_new_attr := jsonb_set(v_new_attr, '{ux,commonColors,radioActiveBgColor}', to_jsonb('#FF7200'::text), true);
        v_updated := true;
        RAISE NOTICE 'radioActiveBgColor added for tenant %', v_tenant_code;
    ELSE
        RAISE NOTICE 'radioActiveBgColor already exists for tenant %', v_tenant_code;
    END IF;

    -- Add/Update radioInActiveBgColor
    IF (v_new_attr #>> '{ux,commonColors,radioInActiveBgColor}') IS NULL THEN
        v_new_attr := jsonb_set(v_new_attr, '{ux,commonColors,radioInActiveBgColor}', to_jsonb('#C9CACC'::text), true);
        v_updated := true;
        RAISE NOTICE 'radioInActiveBgColor added for tenant %', v_tenant_code;
    ELSE
        RAISE NOTICE 'radioInActiveBgColor already exists for tenant %', v_tenant_code;
    END IF;
