-- ============================================================================
-- 🚀 Script    : Rollback flag "enableSelfAttestationCompletion" in tenant_attr column for given tenants
-- 📌 Purpose   : Rollback specific tenants with the flag for UI behavior
-- 🧑 Author    : Siva Krishna
-- 📅 Date      : 03-11-2025
-- 🧾 Jira      : RES-1027(subtask)
-- ⚠️ Inputs    : Array of tenant codes (update only those tenants)
-- 📤 Output    : Rollback the flag "enableSelfAttestationCompletion"=false in tenant_attr for given tenants
-- 🔗 Script URL: NA
...
BEGIN
    RAISE NOTICE '[Information] Starting targeted Rollback of "enableSelfAttestationCompletion" flag for selected tenants...';
    ...
    LOOP
        ...
        IF v_exists THEN
            ...
            RAISE NOTICE '[Updated] Tenant % - Flag existed; set to False.', v_tenant.tenant_code;
        ELSE
            -- Flag missing, add with False
            UPDATE tenant.tenant
            SET tenant_attr = tenant_attr || jsonb_build_object('enableSelfAttestationCompletion', false)
            WHERE tenant_code = v_tenant.tenant_code
              AND delete_nbr = 0;
