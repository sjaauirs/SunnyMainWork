...
--               with value `true` to the `tenant_attr` column in `tenant.tenant` table to make terms
...
BEGIN
  ...
  SET tenant_attr = tenant_attr || jsonb_build_object('enableActivityTracking', true)
