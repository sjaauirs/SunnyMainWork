...
-- 📌 Purpose   : To rollback the colour codes added for WATCO-TENANT-CODE in tenant_attr 
-- 🧑 Author    : Siva Krishna
-- 📅 Date      : 2025-09-22
-- 🧾 Jira      : RES-550
...
DO $$
DECLARE
    v_tenant_code TEXT := '<WATCO-TENANT-CODE>';  -- Replace with your tenant_code
    v_tenant_id BIGINT;
    v_tenant_attr JSONB;
BEGIN
    -- Step 1: Fetch tenant
    ...
    IF v_tenant_attr->'ux' ? 'taskTileColors' THEN
        v_tenant_attr := jsonb_set(
            v_tenant_attr,
            '{ux,taskTileColors}',
            (v_tenant_attr->'ux'->'taskTileColors') - 'borderColor' - 'actionsBgColor'
        );
