-- ============================================================================
-- 🚀 Script    : Script to Update flag "displayAlternateDialer" in tenant_attr column with default value TRUE
-- 📌 Purpose   : Based on this flag UI will flip the dialer in UI
-- 🧑 Author    : Siva Krishna
-- 📅 Date      : 24-09-2025
-- 🧾 Jira      : RES-150 & RES-560(Sub-task)
-- ⚠️ Inputs    : HAP-TENANT-CODE
-- 📤 Output    : It will Update the flag "displayAlternateDialer" to 'true' for input tenant in tenant.tenant
-- 🔗 Script URL: NA
-- 📝 Notes     : NA
-- ============================================================================
...
DECLARE
    ...
    v_exists BOOLEAN;
BEGIN
    RAISE NOTICE '[Information] Starting update: Attempting to set "displayAlternateDialer"=true for tenant_code=%', v_tenant_code;
    ...
    SET tenant_attr = tenant_attr || jsonb_build_object('displayAlternateDialer', true)
    ...
    IF v_updated_count > 0 THEN
        RAISE NOTICE '[Information] Successfully updated "displayAlternateDialer"=true for tenant_code=%', v_tenant_code;
    ELSE
