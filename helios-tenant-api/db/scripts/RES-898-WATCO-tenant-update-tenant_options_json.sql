...
-- 📌 Purpose   : Add or update `primaryMenu` and `secondaryMenu` inside tenant_option_json.benefitsOptions
-- 🧑 Author    : Siva Krishna Reddy
-- 📅 Date      : 2025-10-17
-- 🧾 Jira      : RES-898
-- ⚠️ Inputs    : WATCO-TENANT-CODE
-- 📤 Output    : Updates JSON structure; logs status messages via RAISE NOTICE
-- 🔗 Script URL: NA
-- 📝 Notes     : 
--   - Preserves existing keys under benefitsOptions.
--   - Adds menus if not present; updates only specific arrays if they exist.
-- ============================================================================

DO
$$
DECLARE
    v_tenant_code   TEXT := '<WATCO-TENANT-CODE>';  -- Replace with target tenant code
    v_json_input    JSONB := '{
        "primaryMenu": ["myRewards"],
        "secondaryMenu": ["officialRules","privacyPolicy","signOut"],
        "showSelectedPrimaryMenuItem": false
    }'::jsonb;
...
BEGIN
    ...
    -- ✅ Step 3: Check if primaryMenu already exists
    ...
    END IF;
     ...
     IF v_flag_exist THEN
        RAISE NOTICE '🔄 showSelectedPrimaryMenuItem exists — updating with input value.';
    ELSE
        RAISE NOTICE '➕ showSelectedPrimaryMenuItem does not exist — adding flag with input value';
    END IF;

    -- ✅ Step 4: Update JSON — only modify relevant menu keys
