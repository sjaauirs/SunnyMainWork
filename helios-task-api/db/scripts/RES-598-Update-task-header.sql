-- ============================================================================
-- 🚀 Script    : Script to update task header for Complete your A1C task
-- 📌 Purpose   : Update task header for comp_your_a1c task
-- 🧑 Author    : Siva Krishna
-- 📅 Date      : 2025-Oct-06
-- 🧾 Jira      : RES-598
...
DECLARE
    ...
    v_new_task_header        TEXT := 'Complete your A1C test';  
    v_task_id                BIGINT;
...
BEGIN
    ...
    IF v_existing_code <> v_new_task_external_code THEN
        UPDATE task.task_reward
        SET task_external_code = v_new_task_external_code
        WHERE tenant_code = v_tenant_code
          AND task_id = v_task_id
          AND delete_nbr = 0;

        RAISE NOTICE '[Information]: task_external_code updated from "%" to "%" for tenant_code=%, task_id=%',
                     v_existing_code, v_new_task_external_code, v_tenant_code, v_task_id;
    ELSE
