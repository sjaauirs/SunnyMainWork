...
namespace SunnyRewards.Helios.User.Api.Controllers
{
    ...
    public class PersonRoleController : ControllerBase
    {
        ...
        public async Task<ActionResult<GetPersonRolesResponseDto>> GetPersonRoles([FromBody] GetPersonRolesRequestDto requestDto)
        {
        ...
        /// <returns></returns>
        [HttpGet("access-control-list/{consumerCode}")]
        public async Task<ActionResult<AccessControlListResponseDTO>> GetAccessControlList(string consumerCode)
        {
            ...
            try
            {
                ...
                var response = await _personRoleService.GetAccessControlList(consumerCode);
                if (response.ErrorCode != null)
                {
                    _logger.LogError("{ClassName}.{MethodName}: API Error Occurred while fetching personRoles, ErrorCode:{ErrorCode}", className, methodName, response.ErrorCode);
                    return StatusCode((int)response.ErrorCode, response);
